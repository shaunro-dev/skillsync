...
exports.sendMessage = async (req, res) => {
  try {
    ...
    if (task.company.toString() !== receiver && (!task.assignedStudent || task.assignedStudent.toString() !== receiver)) {
      return res.status(403).json({ message: "Invalid receiver for this task" });
    }

    const message = await Message.create({ sender: req.user.id, receiver, taskId, content });
...
exports.getConversations = async (req, res) => {
  try {
    ...
    res.json(conversations.filter(c => c !== null).sort((a, b) => {
...
exports.markAsRead = async (req, res) => {
  try {
    const { taskId } = req.params;
    await checkTaskAccess(taskId, req.user.id);
    await Message.updateMany({ taskId, receiver: req.user.id, read: false }, { $set: { read: true } });
